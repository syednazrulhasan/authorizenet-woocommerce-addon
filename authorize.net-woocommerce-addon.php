...
if(class_exists('WC_Payment_Gateway'))
{
	class WC_Authorizenet_Gateway extends WC_Payment_Gateway 
	{
		...
		$this->authorizenet_lineitems       = $this->get_option( 'authorizenet_lineitems' );
		...
		'authorizenet_lineitems' => array(
		 'title'       => __( 'Authorize.Net + Line Items', 'woocommerce' ),
		 'type'        => 'checkbox',
		 'label'       => __( 'Enable Sending of Line Items to Authorize.Net.<span style="color:red;">Make sure to keep <b>Unchecked</b> if your line items is reaches the count more than 30.</span>', 'woocommerce' ),
		 'description' => __( 'If checked line items will be sent to authorize.net', 'woocommerce' ),
		 'desc_tip'      => true,
		 'default'     => 'no',
		),
		...
		if($this->authorizenet_lineitems == 'yes'){
		$i =1;
